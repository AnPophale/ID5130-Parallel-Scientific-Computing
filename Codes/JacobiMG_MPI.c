//Parallel Jacobi Method with Multigrid using MPI
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
#include<mpi.h>
...
int main(){
    ...
    int N1 = 33, N2 = 17;
    int pre = 5; //Number of smoothing iterations
    ...
    phi_old = allocate2DArray(N1_local, N1);
    ...
    delx2_old = allocate2DArray(N2_local, N2);
    q = allocate2DArray(N1_local, N1);

    //Row decomposition for fine mesh
    ...
    while(err > 0.0001){
        ...
        for(k = 0; k<pre; k++){
            ...
            if(my_id != size-1)
                MPI_Send(phi_old[N1_local-1],N1,MPI_DOUBLE,my_id+1,tag1,MPI_COMM_WORLD);
            if(my_id != 0){
            ...
            if(my_id !=0)
                MPI_Send(phi_old[0],N1,MPI_DOUBLE,my_id-1,tag2,MPI_COMM_WORLD);
            if(my_id != size-1){
                MPI_Recv(&phi_bot,N1,MPI_DOUBLE,my_id+1,tag2,MPI_COMM_WORLD,&status);
                for(j = 1; j<N1-1; j++){
                        phi[N1_local-1][j] = 0.25*(phi_old[N1_local-2][j] + phi_bot[j] + phi_old[N1_local-1][j+1] + phi_old[N1_local-1][j-1] - delta1*delta1*q[N1_local-1][j]);
                }
            }
        ...
        for(k = 0; k<pre; k++){
            ...
            //Sending the ghost nodes required at the top from i-1th processor to ith processor
            if(my_id != size-1)
                MPI_Send(delx2_old[N2_local-1],N2,MPI_DOUBLE,my_id+1,tag1,MPI_COMM_WORLD);
            if(my_id != 0){
                MPI_Recv(&delx2_top,N2,MPI_DOUBLE,my_id-1,tag1,MPI_COMM_WORLD,&status);
                for(j = 1; j<N2-1; j++){
                        delx2[0][j] = 0.25*(delx2_top[j] + delx2_old[1][j] + delx2_old[0][j+1] + delx2_old[0][j-1] - delta2*delta2*res2[0][j]);
                }
            }

            //Sending the ghost nodes required at the bottom from i+1th processor to ith processor
            if(my_id !=0)
                MPI_Send(delx2_old[0],N2,MPI_DOUBLE,my_id-1,tag2,MPI_COMM_WORLD);
            if(my_id != size-1){
                MPI_Recv(&delx2_bot,N2,MPI_DOUBLE,my_id+1,tag2,MPI_COMM_WORLD,&status);
                for(j = 1; j<N2-1; j++){
                        delx2[N2_local-1][j] = 0.25*(delx2_old[N2_local-2][j] + delx2_bot[j] + delx2_old[N2_local-1][j+1] + delx2_old[N2_local-1][j-1] - delta2*delta2*res2[N2_local-1][j]);
                }
            }
            ...
            for(i = 1; i<N2_local-1; i++){
                for(j = 1; j<N2-1; j++){
                        delx2[i][j] = 0.25*(delx2_old[i+1][j] + delx2_old[i-1][j] + delx2_old[i][j+1] + delx2_old[i][j-1] - delta2*delta2*res2[i][j]);  
                }
            }
        }
    ...
    if(my_id ==0){
        printf("Number of iterations for Parallel Jacobi Method with Multigrid using MPI are : %d\n",2*pre*iter);
    ...
    deallocate2DArray(phi_old, N1_local);
    ...
    deallocate2DArray(delx2_old, N2_local);
